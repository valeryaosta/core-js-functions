...
function getPolynom(...coefficients) {
  ...
  return function polynomFunction(x) {
    return coefficients.reduce((sum, currentCoeff, index) => {
      const power = coefficients.length - index - 1;
      return sum + currentCoeff * x ** power;
    }, 0);
  };
}
...
function memoize(func) {
  ...
  return function checkIfIsCached() {
    if (isCached) {
      return cache;
    }
...
function logger(func, logFunc) {
  return (...args) => {
    const argsRepresentation = args.map((arg) => JSON.stringify(arg)).join(',');
    logFunc(`${func.name}(${argsRepresentation}) starts`);
    const operationResult = func(...args);
